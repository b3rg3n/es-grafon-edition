...
label day3_main1:
    $ backdrop = "days"
    $ new_chapter(3, translation_new["day3"])
    $ day_time()
    scene bg black
    window show
    ...
    window hide
    menu:
        "Извини, но я уже с Леной договорился" if True:
            $ day3_breakfast_with_un = 1
            $ lp_un = lp_un + 1
            jump day3_breakfast_un
        "Ладно, подожди минутку" if True:
...
label day3_main2:
    ...
    window hide
    menu:
        "Хорошо, я приду..." if True:
            $ day3_un_help_accept = 1
            $ lp_un = lp_un + 1
            jump day3_helpaccept
        "Знаете, меня Ольга Дмитриевна попросила вечером ей помочь..." if True:
...
label day3_main3:
    ...
    stop ambience fadeout 2
    menu:
        "Стоит пойти посмотреть" if True:
            $ lp_dv = lp_dv + 1
            stop music fadeout 2
            jump day3_stage_dv
        "Какая разница? Надо продолжить искать ответы" if True:
            $ day3_house_of_mt = 1
            window show
...
label day3_house_of_mt:
    ...
    with dissolve
    menu:
        "Пожалуй, я помогу Славе" if True:
            $ day3_sl_cleaned = 1
            $ lp_sl = lp_sl + 1
            jump day3_square
        "Думаю, помогу ребятам в постройке гигантских роботов" if True:
            jump day3_clubs
        "Ладно, я помогу спортивному клубу" if True:
            $ day3_us_football = 1
            $ lp_us = lp_us + 1
...
label day3_square:
    ...
    if _preferences.language != None:
        $ renpy.notify(translation_new["note9"])
...
label day3_clubs:
    ...
    with dissolve
    window show
    "Может быть, не таких уж и гигантских."
...
label day3_main4:
    ...
    menu:
        ...
        "Остаться и помочь Ульяне убраться" if True:
            $ day3_us_cleaned = 1
            $ lp_us = lp_us + 1
...
label day3_main5:
    ...
    if day3_dv_accept == 1:
        ...
        if day3_un_help_accept == 1:
            ...
            window hide
            menu:
                "Пойти к Алисе" if True:
                    $ lp_un = lp_un - 5
                    $ day3_un_dumped = True
                    stop ambience fadeout 2
                    jump day3_evening_dv
                "Пойти на дискотеку" if True:
                    $ day3_dv_dumped = True
                    $ lp_dv = lp_dv - 5
        elif True:
            window hide
...
label day3_evening_dv:
    ...
    window hide
    menu:
        "Рассказать, что был с Алисой" if True:
            window show
            me "По правде говоря, я с Алисой был на сцене."
            mt "И что же вы там делали?"
            me "Играли на гитаре…"
        "Придумать другое оправдание" if True:
            window show
            $ lp_dv = lp_dv + 1
            me "Я…{w} гулял."
            mt "Один?"
...
label day3_fail:
    ...
    if _preferences.language == "spanish":
        $ urhere = "Estás aquí por una razón"
    elif _preferences.language == "italian":
        $ urhere = "Sei qui per un motivo"
    elif _preferences.language == "chinese":
        $ urhere = "你来到这里是有原因的"
    elif _preferences.language == "english":
        $ urhere = "You are here for a reason"
    elif _preferences.language == "french":
        $ urhere = "Tu es là pour une raison"
    elif True:
        $ urhere = "Ты здесь не просто так"
    show urhere1 urhere:
        xpos -0.5
        ypos 0.3
        linear 4.0 xpos 1.5
