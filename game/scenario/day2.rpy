...
label day2_main1:
    $ backdrop = "days"
    $ new_chapter(2, translation_new["day2"])
    $ day_time()
    scene bg black
    $ renpy.pause(2)
    window show
...
label day2_main2:
    ...
    if _preferences.language != None:
        $ renpy.notify(translation_new["note6"])
    ...
    window hide
    menu:
        "Пойти за картами со Славей" if True:
            $ day2_cards_with_sl = 1
            $ lp_sl = lp_sl + 1
            jump day2_cards_with_sl
        "Пойти одному" if True:
...
label day2_pre_cards:
    ...
    if _preferences.language != None:
        $ renpy.notify(translation_new["note7"])
    ...
    window hide
    menu:
        "Поспорить с Алисой" if True:
            $ day2_dv_bet = 1
            $ lp_dv = lp_dv + 2
            $ lp_un = lp_un - 1
            window show
        "Не спорить с Алисой" if True:
            $ day2_dv_bet = 0
            $ lp_dv = lp_dv - 2
            $ lp_un = lp_un + 1
            window show
...
label day2_cards:
    ...
    if _preferences.language != None:
        $ renpy.notify(translation_new["note8"])
...
label day2_cardgame:
    ...
    if persistent.CardsDemo:
        menu:
            "Пройти обучение" if True:
                jump demo_play
            "Пропустить обучение" if True:
...
label demo_play:
    python:
        ...
        rival = CardGameRivalUn(un_avatar_set, translation_new["test_game"])
    jump cards_gameloop
...
label day_2_cards_continue:
    ...
    if persistent.CardsWon3:
        menu:
            "Играть турнир" if True:
                jump un_play
            "Пропустить турнир (выиграть у Алисы)" if True:
                jump dv_play_win
            "Пропустить турнир (проиграть Алисе)" if True:
                jump dv_play_fail
            "Пропустить турнир (проиграть Ульяне)" if True:
                jump us_play_fail
            "Пропустить турнир (проиграть Лене)" if True:
                jump un_play_fail
...
label un_play:
    python:
        ...
        generate_cards("bg hall",dialogs)
        rival = CardGameRivalUn(un_avatar_set, translation_new["un"])
    jump cards_gameloop
label un_play_fail:
    $ renpy.block_rollback()
...
label us_play:
    python:
        ...
        generate_cards("bg hall",dialogs)
        rival = CardGameRivalUs(us_avatar_set, translation_new["us"])
    jump cards_gameloop
...
label dv_play:
    python:
        ...
        generate_cards("bg hall",dialogs)
        rival = CardGameRivalDv(dv_avatar_set, translation_new["dv"])
    jump cards_gameloop
label dv_play_draw:
    $ show_cards()
    window show
