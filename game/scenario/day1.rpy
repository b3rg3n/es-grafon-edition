...
label day1:
    $ backdrop = "days"
    $ new_chapter(1, translation_new["day1"])
    $ day_time()
    scene bg black
    ...
    if _preferences.language != None:
        $ renpy.notify(translation_new["note1"])
    ...
    slp "Привет, ты, наверное, только что приехал?"
    window hide
    menu:
        "Не отвечать ей" if True:
            window show
    ...
    show sl normal pioneer at center with dspr
    slp "Тебе к вожатой надо, она всё расскажет!"
    slp "Смотри.{w} Сейчас идёшь прямо-прямо, доходишь до площади, затем налево, дальше будут домики."
    "Она показала в сторону ворот, как будто я знал, что за ними."
    if _preferences.language != None:
        $ renpy.notify(translation_new["note2"])
    ...
    slp "Я же тебе сказала повернуть налево на площади, а ты куда пошёл?"
    "Пионерская форма на ней сменилась на купальник."
    slp "Ой, я же так и не представилась!{w} Меня Славя зовут!"
    slp "Вообще, полное имя Славяна, но все меня Славей зовут.{w} И ты тоже зови!"
    me "А… да…"
    ...
    show us laugh sport far behind sl at right with dspr
    ...
    stop ambience fadeout 2
    if _preferences.language != None:
        $ renpy.notify(translation_new["note4"])
    ...
    if _preferences.language != None:
        $ renpy.notify(translation_new["note5"])
    ...
    window hide
    menu:
        "Попытаться отнять котлету" if True:
            $ lp_us += 1
            window show
            "Я грозно посмотрел на неё и уже было вытянул вперёд руку..."
        "Не пытаться отнять котлету" if True:
            window show
    ...
    window hide
    menu:
        "Похвалить книгу" if True:
            $ lp_un += 1
            window show
            me "Хорошая книжка…"
            un "Спасибо."
        "Ничего не говорить" if True:
            window show
            pass
    ...
    "Загадка Лены заняла своё место в массивном списке загадок этого лагеря, который я вёл у себя в голове."
    stop music fadeout 5
    "..."
    window hide
    with fade2
